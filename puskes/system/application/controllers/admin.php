...
class Admin extends Controller {
	 	...
	 	function rekabobat()
	{
		...
		if($data["status"]=="Administrator"){
			...
			$this->load->model('Admin_model');
			$this->load->library('Pagination');
            $data["kategori"] = $this->Admin_model->Kategori($id_kategori);
             $data["kat"] = $this->Admin_model->Kat_Rekap();
		...
		function cekstok()
	{
		...
		if($data["status"]=="Administrator"){
			...
			$tot_hal = $this->Admin_model->Total_Stok();
	...
	function simpanobat()
	{
		...
		$status=$pecah[2];
			...
			$this->Admin_model->Simpan_Obat($in);
   ...
   function editobat()
	{
	   $data = array();
		...
		else
		{
			...
			$page=$this->uri->segment(3);
			$this->load->model('Admin_model');
			$data['ed']=$this->Admin_model->Edit_Obat($id);
			$data['provinsi'] = $this->Admin_model->Get_Provinsi();
	...
	function updateobat()
	{
		...
		$status=$pecah[2];
			...
			$this->Admin_model->Update_Obat($in);
	...
	function hapusobat()
	{
		...
		else
		{
			...
			if($status=="Administrator"){
			$this->load->model('Admin_model');
			$this->Admin_model->Delete_Obat($id);
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/admin/obat'>";
			}
			else{
			?>
   ...
   function inputobat()
	...
	function simpankatobat()
	{
		...
		if($data["status"]=="Administrator"){
			...
			$this->Admin_model->Simpan_Kat_obat($in);
	...
	function editkatobat()
	{
		...
		else
		{
			...
			$this->load->model('Admin_model');
			$data['ed']=$this->Admin_model->Edit_Kat_Obat($id);
	   		$data['scriptmce'] = $this->scripttiny_mce();
			$data["tanggal"] = mdate($datestring, $time);
			$this->load->view('admin/bg_atas',$data);
			$this->load->view('admin/bg_menu');
	...
	function updatekatobat()
	{
		...
		$status=$pecah[2];
			...
			$in['id_kategori']=$this->input->post('id_kategori');
			$in['kategori']=$this->input->post('kategori');
			$this->Admin_model->Update_Kat_Obat($in);
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/admin/katobat'>";
			}
			else{
			?>
	...
	function hapuskatobat()
	{
		...
		else
		{
			...
			$this->load->model('Admin_model');
			$this->Admin_model->Hapus_Kat_Obat($id);
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/admin/katobat'>";
			}
			else{
			?>
   ...
   //============SUB KATEGORI OBAT=============================================================
	...
	function simpansub()
	{
		...
		if($data["status"]=="Administrator"){
			...
			$this->Admin_model->Simpan_Sub($in);
	...
	function editsub()
	{
		...
		else
		{
			...
			$this->load->model('Admin_model');
			$data['kategori']=$this->Admin_model->Kat_Obat();
			$data['ed']=$this->Admin_model->Edit_Sub($id);
	   		$data['scriptmce'] = $this->scripttiny_mce();
			$data["tanggal"] = mdate($datestring, $time);
			$this->load->view('admin/bg_atas',$data);
			$this->load->view('admin/bg_menu');
	...
	function updatesub()
	{
		...
		$status=$pecah[2];
			...
			$in['id_kategori']=$this->input->post('kategori');
			$in['nama_sub']=$this->input->post('nama_sub');
			$this->Admin_model->Update_Sub($in);
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/admin/subkategori'>";
			}
			else{
			?>
   ...
   function akses()
	{
		...
		$data["tanggal"] = mdate($datestring, $time);
			if($data["status"]=="Administrator"){
			$tot_hal = $this->Admin_model->Total_Artikel("tbl_akses");
			...
			$data["page"] = $page;
			$data["akses"] = $this->Admin_model->Daftar_Akses($offset,$limit);
			$this->load->view('admin/bg_atas',$data);
			$this->load->view('admin/bg_menu');
		...
		$data["status"]=$pecah[2];
	   	$data['scriptmce'] = $this->scripttiny_mce();
			if($data["status"]=="Administrator"){
				...
				$this->Admin_model->Simpan_Akses($in);
	...
	function editakses()
	{
		...
		$data["tanggal"] = mdate($datestring, $time);
			...
			$time = time();
			$data["wkt_skr"] = mdate($tgl,$time);
			$data["detail"]=$this->Admin_model->Edit_Content("tbl_akses","id_user=".$id."");
			$this->load->view('admin/bg_atas',$data);
			$this->load->view('admin/bg_menu');
	   	...
	   	$data['scriptmce'] = $this->scripttiny_mce();
			if($data["status"]=="Administrator"){
				...
				if($pass!="")
				{
					...
					$this->Admin_model->Update_Content("tbl_akses",$in,"id_user");
					$this->Admin_model->Update_Password($pass,$in["id_user"]);
					echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/admin/akses'>";
				}
				else{
					$in["nip"]=$this->input->post('nip');
	...
	function hapusakses()
	{
		...
		$data["status"]=$pecah[2];
	   	$data['scriptmce'] = $this->scripttiny_mce();
			if($data["status"]=="Administrator"){
			$this->Admin_model->Delete_Content($id,"id_user","tbl_akses");
			?>
	...
	function cek_stok()
	{
	   $data = array();
		...
		else
		{
			...
			$page=$this->uri->segment(3);
			$this->load->model('Admin_model');
            $data["detail"]=$this->Admin_model->Detail_Transaksi($id_obat);
			$data['provinsi'] = $this->Admin_model->Get_Provinsi();
			$data['kota'] = $this->Admin_model->Get_Kota();
            $this->Admin_model->Update_Counter($id_obat);
	   		$data['scriptmce'] = $this->scripttiny_mce();
			$this->load->view('admin/bg_atas',$data);
			$this->load->view('admin/bg_menu');
    ...
    function simpantransaksi()
	{
		...
		$status=$pecah[2];
			...
			$in["jenis_kelamin"]=strip_tags($this->input->post('jenis_kelamin'));
			$in["tanggal"]=$this->input->post('tanggal');
            $in["jumlah"]=strip_tags($this->input->post('jumlah'));
			$stok=strip_tags($this->input->post('jumlah'));
			$this->Admin_model->Simpan_Artikel("tbl_transaksi",$in);
			$this->Admin_model->Update($in,$stok);
 ...
 function pdf_data()
   ...
   function pdf_data_jamkesmas()
    ...
    function pdf_data_askesda()
	{
        ...
        $id_kategori=$this->input->post('id_kategori');
        $data['data']=$this->Admin_model->Ambildata($id_kategori);
        $title=array(
